...
class InitialsSensitivity(object):
    ...
    def run(self, save_name=None, out_dir=None, num_processors=20):
        """
        ...
        traj = self._solver.run(param_values=self.params_to_run, num_processors = num_processors)
    ...
    def create_heatplot(self, x_axis_label=None, save_name=None,
        ...
        color_bar = plt.colorbar(im, ax=ax, orientation='horizontal', ticklocation = 'top')
        color_bar.set_label(r'% change', labelpad=5)
...
#         ax.set_xlim(v_min - 2, v_max + 2)
    ...
    def create_swarmplot(self, x_axis_label=None, save_name=None,
                                    out_dir=None, show=False):
        """
        Heat map and box plot of sensitivities
        ...
        v_min = -1 * v_max
        x = [np.array(mat).flatten() for mat in sens_ij_nm[::-1]]
        plt.swarmplot(x) #, vert=False, labels=None, showfliers=True, whis='range')
        ax.set_xlim(v_min - 2, v_max + 2)
        if x_axis_label is not None:
